...
public class DieResultTests
{
    ...
    {
        var d1 = new DieResult(4, 1, 7);
        var d2 = new DieResult(7, 5, 11);

        var result = d1 + d2;

        result.Hits.Should().Be(d1.Hits + d2.Hits);
        result.Ones.Should().Be(d1.Ones + d2.Ones);
        result.DiceRolled.Should().Be(d1.DiceRolled + d2.DiceRolled);
    }
    ...
    {
        var glitch1 = new DieResult(0, 2, 3);
        var glitch2 = new DieResult(0, 2, 4);
        var safe1 = new DieResult(0, 2, 5);
        var safe2 = new DieResult(0, 2, 6);
    ...
    public void RollResultCalculatesCorrectly()
    {
        ...
        var criticalGlitch = new DieResult(1, 2, 4);
        var failure = new DieResult(1, 1, 4);
        var glitch = new DieResult(2, 2, 4);
        var success = new DieResult(2, 1, 4);
